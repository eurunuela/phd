...
var handlers = {
  ...
  block(node, state) {
    ...
    if (node.visibility === "remove")
      return;
    if ((_e = (_d = node.data) === null || _d === void 0 ? void 0 : _d.tags) === null || _e === void 0 ? void 0 : _e.includes("no-tex"))
      return;
    state.renderChildren(node, false);
  },
  ...
  code(node, state) {
    ...
    let end = "\n\\end{verbatim}";
    if (state.options.codeStyle === "listings" || getClasses2(node.class).includes("listings") && node.lang !== void 0) {
      state.usePackages("listings");
      start = `\\begin{lstlisting}[language=${node.lang}]
`;
      end = "\n\\end{lstlisting}";
    } else if (state.options.codeStyle === "minted" || getClasses2(node.class).includes("minted")) {
...
//# sourceMappingURL=/build/_shared/dist-TWFBQNOA.js.map
