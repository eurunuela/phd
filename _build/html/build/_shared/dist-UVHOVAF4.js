...
} from "/build/_shared/chunk-XPFBZO6U.js";
import {
  fileError,
  select,
  selectAll,
  ...
  "../../node_modules/color/index.js"(exports, module) {
    ...
    for (const model of Object.keys(convert2)) {
      ...
      Color3.prototype[model] = function(...args) {
        ...
        return new Color3([...assertArray(convert2[this.model][model].raw(this.color)), this.valpha], model);
      };
...
function stringToLatexText(text) {
  const escaped = (text !== null && text !== void 0 ? text : "").replace(/\\ /g, BACKSLASH_SPACE).replace(/\\/g, BACKSLASH).replace(/\/\//g, COMMENT).replace(/~/g, TILDE);
  ...
  const final = replaced.replace(new RegExp(BACKSLASH_SPACE, "g"), "\\\\ ").replace(new RegExp(BACKSLASH, "g"), "\\\\").replace(new RegExp(COMMENT, "g"), "\\/\\/").replace(new RegExp(TILDE, "g"), "#sym.tilde");
...
var unifiedLatexAttachMacroArguments = function unifiedLatexAttachMacroArguments2(options) {
  return (tree) => {
    const { macros: macros17 = {} } = options || {};
    if (Object.keys(macros17).length === 0) {
      console.warn(
...
function writeTypst(node, state = new State()) {
  if (node.type === "whitespace") {
    ...
    state.write(convertText(node.content));
...
function addMacrosToState(value, state) {
  ...
  Object.entries(state.options.math).forEach(([k, v]) => {
    ...
    if (value.includes(key))
      state.data.mathPlugins[key] = texToTypst(v);
  });
}
...
var blockquote = `#let blockquote(node, color: gray) = {
  let stroke = (left: 2pt + color.darken(20%))
...
var handlers = {
  ...
  code(node, state) {
    var _a;
    const start = `\`\`\`${(_a = node.lang) !== null && _a !== void 0 ? _a : ""}
`;
    const end = "\n```";
    state.write(start);
...
//# sourceMappingURL=/build/_shared/dist-UVHOVAF4.js.map
